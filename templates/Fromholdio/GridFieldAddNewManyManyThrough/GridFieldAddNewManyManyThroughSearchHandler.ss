...
<div class="add-new-manymanythrough-search-results">
	<% if $Items %>
		<ul class="list-group add-new-manymanythrough-search-items" data-add-link="$Link('add')">
			<% loop $Items %>
				<li class="$EvenOdd list-group-item list-group-item-action">
                    <a href="#" data-id="$Result.ID">$Title</a>
                </li>
			<% end_loop %>
		</ul>
	<% else %>
	...
	<% if $Items.MoreThanOnePage %>
		<ul class="pagination add-new-manymanythrough-search-pagination">
